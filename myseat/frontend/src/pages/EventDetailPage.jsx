...
function EventDetailPage() {
  ...
  const groupedSeats = useMemo(() => {
    ...
    seats.forEach((seat) => {
      ...
      grouped.get(sectionKey).push(seat)
    })
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border bg-white p-4 shadow-sm">
        ...
        <div className="space-y-4">
          {groupedSeats.map(([sectionId, sectionSeats]) => (
            <div key={sectionId}>
              <p className="mb-2 text-sm font-medium text-slate-700">Section #{sectionId}</p>
              <div className="grid grid-cols-4 gap-2 sm:grid-cols-8 md:grid-cols-10">
                {sectionSeats.map((seat) => {
                  const isSelected = selectedSeatIds.includes(seat.id)
                  const baseClass = 'rounded border px-2 py-2 text-xs font-medium'
                  ...
                  return (
                    ...
                    >
                      R{seat.rowNumber} C{seat.columnNumber}
                    </button>
                  )
                })}
              </div>
